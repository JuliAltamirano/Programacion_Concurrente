...
public class Main {
	...
	private static String ROOT = "C:/Users/nachoaichino/Desktop/FACULTAD/Programacion_Concurrente-master/Trabajo_Practico_3/Codigo_TP3";
	...
	private static String TEMP_DIR ="/t_temporales.xls";
	...
	public static void main(String[] args) {
		...
		RedDePetri rdp = new RedDePetri(
				...
				imp_datos.creaVector(TEMP_DIR,ROOT, NUM_TRANS),
		...
		control_on[0] = new ControlOn(buffer[0], monitor, inv_t_log);
		control_on[1] = new ControlOn(buffer[1] , monitor, inv_t_log);

		ControlActive control_active[] = new ControlActive[2];
		control_active[0] = new ControlActive(control_on[0], monitor, buffer[0], inv_t_log);
		control_active[1] = new ControlActive(control_on[1], monitor, buffer[1], inv_t_log);

		Politica politica = new Politica(buffer);
		
		GeneradorDeDatos generador= new GeneradorDeDatos(monitor, NUM_DATOS); //hilo generador de datos
		
		ArrivalDato arr_dato[]= new ArrivalDato[2];
		arr_dato[0]= new ArrivalDato(politica, monitor, buffer[0]); //hilo de arrivo de datos a buffer1
		arr_dato[1]= new ArrivalDato(politica, monitor, buffer[1]);	//hilo de arrivo de datos a buffer 2
